...
# (https://github.com/HolmesLab/2020_NatComm_interneurons_cortical_function_schizophrenia/blob/master/scripts/03_map_ahba_ctx_to_surface.py)
...
data_path = '/gpfs/milgram/project/holmes/XihanZhang_xz555/gradient_shift/data'
...
ahba_samples = pd.read_csv(os.path.join(data_path, 'ahba/ahba_sampleInfo_reannot.csv'))
RAS_df       = ahba_samples
RAS_df['R']  = np.nan
RAS_df['A']  = np.nan
RAS_df['S']  = np.nan
...
for donor_num in donor_arr:
    ...
    for hemi in ['left', 'right']:
        ...
        RAS_df.loc[donor_ctx_hemi_idxs, 'vertex'] = vertex_mapping.values
...
RAS_df.to_csv(os.path.join(data_path, 'ahba/sample_info_vertex_reannot_mapped.csv'), index=False)
